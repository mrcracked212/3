...
const DEFAULT_TRIAL_DURATION = "1h";
...
const DEFAULT_PACKAGES = {
  trial: {
    ...
    promo_limit: 3,
    required_targets: Number(config.REQUIRED_TRIAL_GROUPS) || 3,
    min_group_members: Number(config.MIN_GROUP_MEMBER_FOR_TRIAL) || 30,
    min_channel_subscribers: Number(config.MIN_CHANNEL_SUBSCRIBER_FOR_TRIAL) || 50,
    only_once: true
  },
  pro: {
    name: "Pro",
...
async function executePromotionJob(job) {
  ...
  const progressIntervalMs = Math.max(2000, Number(config.PROMO_PROGRESS?.UPDATE_INTERVAL_MS) || 3000);
  const progressRetryDelayMs = Math.max(500, Number(config.PROMO_PROGRESS?.RETRY_DELAY_MS) || 1500);
  const progressMaxRetry = Math.max(1, Number(config.PROMO_PROGRESS?.MAX_RETRY) || 3);
...
async function tryActivateTrialForUser(userId) {
  ...
  const requiredTargets = getTrialRequiredTargets();
...
async function registerGroupWhenBotAdded(msg) {
  ...
  await sendSafeMessage(msg.chat.id, [
    ...
    "Grup ini otomatis menjadi target promosi jika bot tetap aktif."
  ].join("\n"));
}
